...
def oligo_batch_info(oligo_ID):
    ...
    sql =   "SELECT batch_number, synthesis_level_ordered, synthesis_level_delivered,\
...
def testlist_oligo(oligo_ID):
    """Returns a tuple of all tests in which the given oligo was used

    Keyword Arguments:
    oligo_ID    -- string, the key of the oligo for which you want retrieve the information."""
    oligo = tuple([oligo_ID]*3)
    sql = "SELECT approval.experiment_ID, test_number, lab_report, experiment_date,\
        oligo_ID_fwd, oligo_ID_rev, oligo_ID_probe,\
        approved_status FROM experiment, approval\
        WHERE experiment.experiment_ID = approval.experiment_ID\
        AND (approval.oligo_ID_fwd = %s \
        OR approval.oligo_ID_rev = %s \
        OR approval.oligo_ID_probe = %s)" % oligo
    return execute_select_queries(sql)
	
def experimentlist_approval():
    """Query for showing a list of all experiments (ordered by date)
        and their results regarding their approval."""

    sql = "SELECT * FROM experiment, approval \
        WHERE experiment.experiment_ID = approval.experiment_ID \
...
def oligolist_test_results():
    ...
    sql = "SELECT DISTINCT oligo.oligo_ID, oligo_name,\
        ...
        COUNT(IF(approved_status='neutral',1,NULL)) AS Neutral,\
        COUNT(IF(approved_status='disapproved',1,NULL)) AS Disapproved,\
        ROUND((COUNT(IF(approved_status='approved',1,NULL))/\
        (COUNT(IF(approved_status='approved',1,NULL))+COUNT(IF(approved_status=\
        disapproved,1,NULL))+COUNT(IF(approved_status='neutral',1,NULL))) *\
        100),0) AS 'Percentage of approval'\
        FROM oligo, approval\
        WHERE (oligo.oligo_ID = approval.oligo_ID_fwd\
...
def supplier_synthesis_quality():
    """Query for showing the average difference in synthesis level per supplier,
    indicating itsquality"""

    sql = "SELECT DISTINCT supplier_name, ROUND(AVG(synthesis_level_ordered -\
            synthesis_level_delivered),2) AS Difference\
            FROM batch, groupwork.`order`, supplier\
            WHERE batch.order_number = groupwork.`order`.order_number\
            AND groupwork.`order`.supplier_ID = supplier.supplier_ID\
            GROUP BY supplier_name"
    return execute_select_queries(sql)

def supplier_delivery_time():
    """Query for calculating the average delivery time per supplier."""

    sql = "SELECT supplier_name, ROUND(AVG(delivery_date - order_date),1)\
            FROM batch, groupwork.`order`, supplier\
            WHERE batch.order_number = groupwork.`order`.order_number AND\
            groupwork.`order`.supplier_ID = supplier.supplier_ID\
            GROUP BY supplier_name"
    return execute_select_queries(sql)

# Project and the linked oligos

def oligos_from_project(project_name):
    ...
    sql = "SELECT oligo.oligo_ID, oligo_name, oligo_type, sequence, description,\
            entry_date, creator, update_date, modifier, label5prime, label3prime,\
            labelM1, labelM1position, notes\
