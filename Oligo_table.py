...
def build_table():
    ...
    db = MySQLdb.connect('127.0.0.1','root', 'root', 'groupwork')
    cursor = db.cursor()
    atttributes_oligo = ['oligo_ID', 'oligo_name', 'oligo_type', '\
                 sequence', 'description', 'entry_date', 'creator', '\
                 update_date', 'modifier', 'label5prime', 'label3prime', '\
                 labelM1', 'labelM1position', 'notes']
    ssw.addColumn('')
    for attribute in atttributes_oligo:
        ssw.addColumn(attribute, 300, align = LEFT)
    query = ("SELECT * FROM Oligo")
